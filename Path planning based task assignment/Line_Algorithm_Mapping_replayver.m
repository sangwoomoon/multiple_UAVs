...
HLI_vel = ones(1,NUM_UAV); %[1 1 1 1 1 1]; % m/s
HLI_max_dyaw = 1*ones(1,NUM_UAV); %[0.4 0.4 0.4 0.4 0.4 0.4]; %rad/s
...
if (OBS_NUM ~= 0)
    vrt = set_obstacle_replayver(vrt,OBS_NUM,OBS_VRT,0,1);
...
if (UNOBS_NUM ~= 0)
    vrt_unknown = set_obstacle_replayver(vrt_unknown,UNOBS_NUM,UNOBS_VRT,OBS_NUM,0);
...
%     [p_task(iter,1),p_task(iter,2)] = ginput(1); % task point
    plot(p_task(iter,1),p_task(iter,2), 'r.','markersize',10);
...
%         end
    ...
    for iter_scan = 1 : 2 : length(search_pts(:,1))-1
        if (search_pts(iter_scan,3)~=7)
              plot(search_pts(iter_scan:iter_scan+1,1),search_pts(iter_scan:iter_scan+1,2),'r-','linewidth',3);
        else
            if (search_pts(iter_scan,4)~=4)
                plot(search_pts(iter_scan:iter_scan+1,1),search_pts(iter_scan:iter_scan+1,2),'r-','linewidth',3);
            end
        end

function out = Task_Assignment6(u) 
...
global chk_range second_choice wpt cnt 
global idxMinTask MinDistTask POS cntt TASK_list TASK_STATUS_prev dis_prev
...
%         end
     ...
     for idx_UAV = 1 : NUM_UAV
        if (dis_prev(1,idx_UAV) == 1 && SetTaskAllocation(1) == 0) 
            SetTaskAllocation = 1 ;
            dis_prev = 0 ; 
        end    
     end
    ...
    if (SetTaskAllocation(1))        
        for iter_UAVx = 1 : NUM_UAV-1
            for iter_UAVy = iter_UAVx+1 : NUM_UAV
                if ((discon(1,iter_UAVx) ~= 1) && (discon(1,iter_UAVy) ~= 1)) 
...
% %                             TASK_STATUS(:,idxMinTask(iter_UAVx)) = [1;iter_UAVx]; % TASK status update.
    ...
    if (sum(discon) == NUM_UAV-1 )
       for idx_UAV = 1 : NUM_UAV
                 if(discon(1,idx_UAV) == 0 )
                            [wpt_temp, second_choice_temp, temp] = ...
                            operate_algorithm(OBS_NUM,OBS_VRT,wpt(iter_wpt(idx_UAV)-1,(idx_UAV-1)*4+1:idx_UAV*4),...
                            vrt_config,vrt_config,...
                            HLI_pos(iter_prcd,(idx_UAV-1)*3+1:(idx_UAV-1)*3+2),p_task(idxMinTask(idx_UAV),:),...
                            iter_wpt(iter_UAVx)-1);
                            wpt(1:length(wpt_temp(:,1)),(idx_UAV-1)*4+1:(idx_UAV*4)) = wpt_temp;
                            second_choice(1,(idx_UAV-1)*4+1:idx_UAV*4) = second_choice_temp;
                            TASK_STATUS(:,idxMinTask(idx_UAV)) = [1;idx_UAV]; % TASK status update.
                             clear wpt_temp second_choice_temp;
                 end
           end
       end
